...
int main(){
    ...
    while(t--){
        ...
        bool p = true;
        for(j=2 ; j<=n ; j++){
            if(h[j] > h[j-1] and h[j] - h[j-1] <= u) continue;
            else if(h[j] < h[j-1] and h[j-1] - h[j] <= d) continue;
            else if(h[j] == h[j-1]) continue;
            else if(h[j] < h[j-1] and h[j-1]-h[j] > d and p) { p = false;continue;}
            else break;
        }
        cout<<j-1<<endl;
    }
